...
class BaseNode
{
    ...
    public function __get($key)
    {
        ...
        return match ($cast) {
            Cast::String
                => $rawValues[0]->{'@value'} ?? (string)$this->resolveValue($rawValues[0]),
            Cast::TranslatableString
                => $this->castToTranslatableString($rawValues),
            Cast::Int
                => (int)($rawValues[0]->{'@value'} ?? 0),
            Cast::Float
                => (float)($rawValues[0]->{'@value'} ?? 0.0),
            Cast::Bool
                => (bool)($rawValues[0]->{'@value'} ?? false),
            Cast::Date
                => $this->asDate($rawValues[0]->{'@value'} ?? null),
            Cast::Collection
                => $this->castToCollection($rawValues, $def['range'] ?? []),
            Cast::Node
                => $this->castToNode($rawValues[0], $def['range'] ?? []),
            default
                => $this->resolveValue($rawValues[0]),
        };
    }
    ...
    protected function isValidType($value, array $allowedRange): bool
    {
        ...
        if (empty($allowedRange)) return true;

        // Check against Allowed Range (Handles both Classes and Primitives like 'string')
        foreach ($allowedRange as $allowed) {
            if (is_object($value) && $value instanceof $allowed) return true;
            if ($allowed === 'string' && is_string($value)) return true;
            if ($allowed === 'int' && is_int($value)) return true;
            if ($allowed === 'float' && is_float($value)) return true;
            if ($allowed === 'bool' && is_bool($value)) return true;
        }
